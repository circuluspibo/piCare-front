...
export default function Dialog({ 
  ...
  children, 
  actions, 
  titleStyle = "text-2xl font-bold mb-4" 
}) {
  ...
  return (
    ...
    >
      <div className="bg-white p-8 rounded-lg shadow-2xl text-center max-w-md w-full animate-fadeInDown">
        ...
        <div className="flex justify-center space-x-4">
          {actions !== undefined && actions.map((action, index) => (
            <button
              ...
              className={`font-bold py-2 px-4 rounded transition duration-200 ${action.style || 'bg-blue-600 text-white hover:bg-blue-700'}`}
            >
              {action.text}
            </button>
          ))}
