...
export default function Main() {
  ...
  // [로직 변경]: useVoiceChat을 상위에서 하나로 관리
  const voiceChat = useVoiceChat({ enableTTS });
  const { playTtsSentence, injectMessage } = voiceChat;
  ...
  const runAutoCaptureProcess = useCallback(async () => {
    ...
    try {
      ...
      if (hbData.cnt_live > 0 && now - lastActionTimeRef.current > 30000) {
        ...
        formData.append("system", PERSONA_SYSTEMS[personaId]);
        ...
        // [로직 수정]: TTS 실행 및 채팅창에 결과 주입
        injectMessage("시스템: 주변 상황 인식", fullText);
      }
    } catch (error) {
      console.error("Auto Mode 에러:", error);
    }
  }, [injectMessage, personaId, isTemp]);
  ...
  const runTestCapture = useCallback(async () => {
    ...
    try {
      ...
      // [로직 수정]: 채팅창에 결과 주입
      injectMessage("테스트 분석 요청", fullText);
    } catch (error) {
      console.error("Test Mode Error:", error);
    } finally {
      setIsTesting(false);
    }
  }, [injectMessage, personaId, isTesting]);
    ...
    <>
      <div className="flex w-full h-full mx-auto p-2 rounded-xl overflow-hidden">
        <div className="w-4/12 flex flex-col items-center justify-between m-1 h-full pb-12">
          <div className="w-full flex-[3] flex items-center justify-center overflow-hidden">
            <img
        ...
        <div className="w-8/12 p-3 bg-white rounded-xl ">
          <div className="text-xl text-gray-600 h-full">
            {/* 로직 수정: voiceChat 객체 전달 */}
            <Prompt voiceChat={voiceChat} />
          </div>
        </div>
